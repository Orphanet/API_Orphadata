...
class TestDisorderController(BaseTestCase):
    ...
    def test_age_by_orphacode(self):
        ...
        response = self.client.open(
            '/age/orphacode/{orphacode}/{language}'.format(orphacode=558, language='EN'),
    ...
    def test_disorder_by_orphacode(self):
        ...
        """
        response = self.client.open(
            '/product1/orphacode/{orphacode}/{language}'.format(orphacode=558, language='EN'),
            method='GET', headers={"ADMIN-API-KEY": "test-key"})
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_epidemiology_by_orphacode(self):
        ...
        """
        response = self.client.open(
            '/epidemiology/orphacode/{orphacode}/{language}'.format(orphacode=558, language='EN'),
            method='GET', headers={"ADMIN-API-KEY": "test-key"})
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_gene_by_disorder_orphacode(self):
        ...
        """
        response = self.client.open(
            '/associatedgene/orphacode/{orphacode}'.format(orphacode=166024),
            method='GET', headers={"ADMIN-API-KEY": "test-key"})
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_phenotype_by_orphacode(self):
        ...
        """
        response = self.client.open(
            '/phenotype/orphacode/{orphacode}/{language}'.format(orphacode=558, language='EN'),
            method='GET', headers={"ADMIN-API-KEY": "test-key"})
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))

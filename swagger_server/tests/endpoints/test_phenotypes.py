...
class TestPhenotypesEndpointsStatus(BaseTestCase):
    ...
    def test_status_200_phenotypes_base(self):
        ...
        """
        url_endpoint = URL_ENDPOINTS.get('phenotypes_base')
    ...
    def test_status_200_phenotypes_orphacodes(self):
        ...
        CHECK:
            - response status is 200
        """
        url_endpoint = URL_ENDPOINTS.get('phenotypes_orphacodes')
        response = self.client.get(url_endpoint)
        self.assert200(response)
    ...
    def test_status_200_phenotypes_by_orphacode(self):
        ...
        CHECK:
            - response status is 200
        """
        url_endpoint = URL_ENDPOINTS.get('phenotypes_by_orphacode').format(self.valid_orphacode)
        response = self.client.get(url_endpoint)
        self.assert200(response)
    ...
    def test_status_404_phenotypes_by_orphacode(self):
        ...
        """
        url_endpoint = URL_ENDPOINTS.get('phenotypes_by_orphacode').format(self.invalid_orphacode)
        response = self.client.get(url_endpoint)
    ...
    def test_status_200_phenotypes_by_hpoids(self):
        ...
        CHECK:
            - response status is 200
        """
        url_endpoint = URL_ENDPOINTS.get('phenotypes_by_hpoids').format(self.valid_hpoids)
        response = self.client.get(url_endpoint)
        self.assert200(response)
    ...
    def test_status_404_phenotypes_by_hpoids(self):
        ...
        CHECK:
            - response status is 404
        """
        url_endpoint = URL_ENDPOINTS.get('phenotypes_by_hpoids').format(self.invalid_hpoid)
        response = self.client.get(url_endpoint)
        self.assert404(response)

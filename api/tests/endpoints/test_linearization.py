...
class TestLineariztionEndpointsStatus(BaseTestCase):
    ...
    def test_status_200_linearization_by_orphacode(self):
        ...
        """
        url_endpoint = self.URL_ENDPOINTS.get('linearization_by_orphacode').format(self.valid_orphacode)
    ...
    def test_status_404_linearization_by_orphacode(self):
        ...
        """
        url_endpoint = self.URL_ENDPOINTS.get('linearization_by_orphacode').format(self.invalid_orphacode)
        response = self.client.get(url_endpoint)
    ...
    def test_status_200_linearization_parents(self):
        ...
        CHECK:
            - response status is 200
        """
        url_endpoint = self.URL_ENDPOINTS.get('linearization_parents')
        response = self.client.get(url_endpoint)
        self.assert200(response)
    ...
    def test_status_200_linearization_by_parent(self):
        ...
        CHECK:
            - response status is 200
        """
        url_endpoint = self.URL_ENDPOINTS.get('linearization_by_parent').format(self.valid_parent)
        response = self.client.get(url_endpoint)
        self.assert200(response)
    ...
    def test_status_404_linearization_by_parent(self):
        ...
        CHECK:
            - response status is 404
        """
        url_endpoint = self.URL_ENDPOINTS.get('linearization_by_parent').format(self.invalid_parent)
        response = self.client.get(url_endpoint)
        self.assert404(response)

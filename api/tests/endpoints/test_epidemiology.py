...
class TestEpidemiologyEndpointsStatus(BaseTestCase):
    ...
    def test_status_200_natural_history_orphacodes(self):
        """test_status_200_epidemiology_orphacodes
        ...
        CHECK:
            - response status is 200
        """
        url_endpoint = self.URL_ENDPOINTS.get('history_orphacodes')
        response = self.client.get(url_endpoint)
        self.assert200(response)

    def test_status_200_natural_history_by_orphacode(self):
        """test_status_200_epidemiology_by_orphacode
        ...
        CHECK:
            - response status is 200
        """
        url_endpoint = self.URL_ENDPOINTS.get('history_by_orphacode').format(self.valid_orphacode)
        response = self.client.get(url_endpoint)
        self.assert200(response)

    def test_status_404_natural_history_by_orphacode(self):
        """test_status_404_epidemiology_by_orphacode
        ...
        CHECK:
            - response status is 404
        """
        url_endpoint = self.URL_ENDPOINTS.get('history_by_orphacode').format(self.invalid_orphacode)
        response = self.client.get(url_endpoint)
        self.assert404(response)

...
class TestClassificationsEndpointsStatus(BaseTestCase):
    ...
    def test_status_200_classification_hchids(self):
        ...
        """
        url_endpoint = self.URL_ENDPOINTS.get('classification_hchids', None)
    ...
    def test_status_404_classification_by_orphacode_and_hchid_10(self):
        ...
        CHECK:
            - response status is 404
        """
        url_endpoint = self.URL_ENDPOINTS.get('classification_by_orphacode_and_hchid').format(self.valid_orphacode, self.invalid_hchid)
        response = self.client.get(url_endpoint)
        self.assert404(response)
    ...
    def test_status_404_classification_by_orphacode_and_hchid_01(self):
        ...
        CHECK:
            - response status is 404
        """
        url_endpoint = self.URL_ENDPOINTS.get('classification_by_orphacode_and_hchid').format(self.invalid_orphacode, self.valid_hchid)
        response = self.client.get(url_endpoint)
        self.assert404(response)
